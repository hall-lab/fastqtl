...
//
// This is modified from R version 3.1.0 code.
// see http://www.r-project.org/Licenses/ for relevant licenses
...
#undef SET_0_swap
    ...
    //                        -----
    double brcmp1(int mu, double a, double b, double x, double y, int give_log)
    {
        /* -----------------------------------------------------------------------
...
#define n_terms_bgrat 30
        ...
        for (int n = 1; n <= n_terms_bgrat; ++n) {
            ...
            if (fabs(dj) <= eps * (sum + l)) {
               ...
               MATHLIB_WARNING4("bgrat(a=%g, b=%g, x=%g,..): did *not* converge; rel.err=%g",a,b,x, fabs(dj) /(sum + l));*/
        }
    ...
    double grat_r(double a, double x, double log_r, double eps)
    {
        /* -----------------------------------------------------------------------
...
#define num_IT 20
    ...
    double esum(int mu, double x, int give_log)
    {
        /* ----------------------------------------------------------------------- */
    ...
    double pf(double x, double df1, double df2, int lower_tail, int log_p)
    {
        ...
        if (df2 == ML_POSINF) {
            ...
            return -1;
            /* We are not defaulting to chisq for infinate degrees of freedom.
             Doing so would have meant importing 1000s of lines of R code so sod it*/
    ...
    {
        if (ISNAN(x) || ISNAN(pin) || ISNAN(qin)) return -1;
